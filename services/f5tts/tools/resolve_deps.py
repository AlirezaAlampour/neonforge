...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "torch/torchvision pinned as constraints, and freeze the final environment."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Output path for the generated torch/torchvision constraints file.",
    )
    parser.add_argument(
        "--requirements",
...
def probe_framework_versions(python_executable: str) -> dict[str, str]:
    ...
    if missing:
        ...
        raise ResolverError(
            ...
            f"is already present. Missing: {names}. Preinstall torch and torchvision "
            "first, then rerun this tool."
        )
...
def run_target(
    ...
    try:
        ...
        missing_module = extract_missing_module(partial_stderr)
        if missing_module:
            process.terminate()
