...
class ReferenceLoader:
    ...
    def load_audio(self, reference_audio: bytes | str, sr: int):
        """
        Load audio from a file path or in-memory bytes without torchaudio.
        """
        source: io.BytesIO | str
        if isinstance(reference_audio, (bytes, bytearray)):
            source = io.BytesIO(reference_audio)
        else:
            path = Path(reference_audio)
            if path.exists():
                source = str(path)
            else:
                source = io.BytesIO(reference_audio.encode("utf-8"))

        audio, original_sr = librosa.load(source, sr=None, mono=True)
        if original_sr != sr:
